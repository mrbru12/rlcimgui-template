...
g++ is used to build ImGui related stuff, but after that everything uses plain gcc, including your source files! 
...
```
> The --recurse-submodules flag is needed to clone all the submodules. If you cloned without it, or _if you created a new repository using the github **template** feature_, then run `git submodule update --init --recursive`.
```bash
